...
export default function UpdateUserForm() {
  const [formDate, setFormData] = useReducer(formReducer, {});
  ...
  if (Object.keys(formDate).length > 0) return <Bug message={"Error"}/>
  ...
  return (
    <form className="grid lg:grid-cols-2 w-4/6 gap-4" onSubmit={handleSubmit}>
      ...
      <button className="flex justify-center text-md w-2/6 bg-yellow-400 text-white px-4 py-2 border-rounded-md hover:bg-gray-50 hover:borer-yellow-500 hover:text-yellow-500 ">
        Update
        <span className="px-1">
          <BiBrush size={24} />
        </span>
